...
export function DashboardLayout({ children }: DashboardLayoutProps) {
    ...
    return (
        <div className="min-h-screen bg-[var(--synapse-bg)] text-[var(--synapse-text)] flex">
            ...
            {/* Main Content Area */}
            <main className="flex-1 md:ml-[260px] min-h-screen flex flex-col transition-all duration-300">
                {/* Top Navbar */}
                <header className="h-16 md:h-20 flex items-center justify-between md:justify-end gap-6 px-4 md:px-10 border-b border-[var(--synapse-border)] bg-[var(--synapse-bg)] sticky top-0 z-40 w-full">
                    
                    {/* Mobile Menu Button */}
                    <button 
                        onClick={() => setIsSidebarOpen(true)}
                        className="p-2 md:hidden text-[var(--synapse-text-muted)] hover:text-[var(--synapse-text)]"
                    >
                ...
                {/* Page Content */}
                <div className="flex-1 p-4 md:p-6 md:px-30">
